...
template <typename T>
CLICK_NOINLINE T *find(T *begin, T *end, const T &val)
{
...
template <typename T>
CLICK_NOINLINE const T *find(const T *begin, const T *end, const T &val)
{
    while (begin < end && *begin != val)
	++begin;
    return begin;
...
template <typename T>
CLICK_NOINLINE T *find(T *collection, const T &val)
{
    T *begin = collection->begin();
    T *end = collection->end();
    while (begin < end && *begin != val)
	++begin;
    return begin;
}
...
template <typename T>
CLICK_NOINLINE const T *find(const T *collection, const T &val)
{
    const T *begin = collection->begin();
